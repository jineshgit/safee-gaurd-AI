...
export default class EvaluationPipeline {
    db: any;
    ...
    static isGibberish(text: string) {
    ...
    static getResponseQuality(text: string) {
        const sentences = text.split(/[.!?]+/).filter(s => s.trim().length > 3);
        const words = text.split(/\s+/).filter(w => w.length > 0);
        const wordCount = words.length;

        let quality: { score: number, issues: string[] } = { score: 0, issues: [] };
    ...
    async evaluate(scenario: any, response: string, persona: any) {
        ...
        if (scenario.id === 'CS-REFUND-POLICY') {
            ...
            if (scenario.forbidden_keywords && Array.isArray(scenario.forbidden_keywords)) {
                scenario.forbidden_keywords.forEach((keyword: string) => {
            ...
            if (scenario.required_keywords && Array.isArray(scenario.required_keywords)) {
                scenario.required_keywords.forEach((keyword: string) => {
                    if (!responseLower.includes(keyword.toLowerCase())) {
